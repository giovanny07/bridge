...
class SolarWindsClient
{
    ...
    public static function fromConnection(Connection $connection): self
    {
        if (($connection->fields['system_type'] ?? '') !== Connection::TYPE_SOLARWINDS) {
            throw new RuntimeException('Only SolarWinds connections can be scanned in this phase.');
        }
    ...
    /**
     * Lightweight connectivity check: one request, no heavy processing.
     * Never throws — returns ['ok' => bool, 'status' => int, 'message' => string,
     * 'latency_ms' => int, 'total' => int].
     */
    public function testConnection(): array
    {
        ...
        try {
            ...
            $latencyMs = (int) round((hrtime(true) - $start) / 1_000_000);
            // Extract HTTP status from the message we control: "SolarWinds returned HTTP NNN: …"
            $status = 0;
    ...
    public function scanIncidents(int $limit = 10): array
    {
        ...
        $response = $this->request('/incidents.json', [
            'per_page' => $limit,
            'page'     => 1,
        ]);
    ...
    /**
     * Samanage uses X-Samanage-Authorization for all token-based auth, not the
     * standard Authorization header. Basic auth is also sent via this header.
     * Custom header auth is left to the user's specification.
     */
    private function buildAuthHeaders(): array
    {
        ...
        // Strip any redundant "Bearer " prefix the user may have pasted in the UI
        $token = $this->secret;
